...
pub fn receive(self: *UdpSocket, buf: []u8) sf.Socket.Error!sf.Socket.ReceivedRaw {
    ...
    try sf.Socket.codeToErr(code);
    return sf.Socket.ReceivedRaw{ .data = buf[0..size], .sender = remote };
}
