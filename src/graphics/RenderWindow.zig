...
pub fn createFromHandle(handle: sf.c.sfWindowHandle, settings: ?sf.ContextSettings) !RenderWindow {
    const c_settings = if (settings) |s| s._toCSFML() else null;
...
pub fn draw(self: *RenderWindow, to_draw: anytype, states: ?sf.RenderStates) void {
    const draw_fn = @field(sf.c, "sfRenderWindow_draw" ++ @TypeOf(to_draw).draw_suffix);
    if (states) |s| {
